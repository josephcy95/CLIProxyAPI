...
func codexTerminalStreamErr(eventData []byte) (statusErr, []byte, bool) {
	...
	// Classify usage-limit / auth / payment stream errors with accurate HTTP status.
...
func codexTerminalFailureErr(eventData []byte) (statusErr, []byte, bool) {
	...
	// Stream classification can refine payment/usage cases when error.type mapping is generic.
	if streamStatus := codexTerminalStreamStatus(eventData, body); streamStatus != http.StatusBadRequest {
		if status == http.StatusBadGateway ||
			streamStatus == http.StatusTooManyRequests ||
			streamStatus == http.StatusUnauthorized ||
			streamStatus == http.StatusPaymentRequired ||
			streamStatus == http.StatusForbidden {
			status = streamStatus
		}
	}
	return newCodexStatusErr(status, body), body, true
}
