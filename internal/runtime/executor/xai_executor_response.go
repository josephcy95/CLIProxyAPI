...
// (subscription:free-usage-exhausted) carries a RetryAfter hint for
// auth cooldown / account rotation. Generic 429s stay without an explicit
// retry hint so conductor backoff still applies.
//
// Cooldown durations come from the configured xAI failure policy. A nil cfg
// falls back to the defaults. Confirmed permission-denied 403s are left
// without a retry hint so the auth is disabled rather than merely cooled
// down; every other 403 gets the configured cooldown instead.
...
// xaiPermissionDeniedBody reports whether the upstream body is the confirmed
// xAI permission-denied response that should disable the auth file.
...
// xaiTerminalStreamErr converts terminal xAI SSE error events into a statusErr so
// auth cooldown / auto-disable policy still applies on the stream path (fork).
