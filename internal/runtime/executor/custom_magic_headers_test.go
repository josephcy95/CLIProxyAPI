...
func TestCustomMagicHeaders_CodexStream(t *testing.T) {
	var gotHeaders http.Header
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotHeaders = r.Header.Clone()
		w.Header().Set("Content-Type", "text/event-stream")
	...
	result, err := executor.ExecuteStream(context.Background(), auth, cliproxyexecutor.Request{
		...
		SourceFormat: sdktranslator.FormatCodex,
		Stream:       true,
		Headers:      http.Header{"Abc": []string{"codex-stream-session-value"}},
	})
	if err != nil {
		t.Fatalf("ExecuteStream() error = %v", err)
	}
	...
	if got := gotHeaders.Get("X-Claude-Code-Session-Id"); got != "codex-stream-session-value" {
		t.Errorf("X-Claude-Code-Session-Id = %q, want %q", got, "codex-stream-session-value")
	}
	if _, exists := gotHeaders["X-Missing"]; exists {
		t.Errorf("expected X-Missing to be omitted, got %q", gotHeaders.Get("X-Missing"))
	}
