...
func TestGetAvailableModelInfosHonorsQuotaAndSuspensionAvailability(t *testing.T) {
	tests := []struct {
		...
		wantInfoAvailable bool
		wantListAvailable bool
	}{
		{
			name:              "quota cooldown remains listed",
			quotaExceeded:     true,
			wantInfoAvailable: true,
			wantListAvailable: true,
		},
		{
		...
		{
			name:              "metadata hides manual suspension while discovery keeps the registered model",
			clientCount:       2,
			quotaExceeded:     true,
			quotaSuspended:    true,
			manualSuspended:   true,
			wantInfoAvailable: false,
			wantListAvailable: true,
		},
	}
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if gotInfoAvailable != testCase.wantInfoAvailable {
				t.Fatalf("GetAvailableModelInfos() available = %v, want %v; models = %#v", gotInfoAvailable, testCase.wantInfoAvailable, infos)
			}

			models := modelRegistry.GetAvailableModels("openai")
			gotListAvailable := len(models) == 1 && models[0]["id"] == modelID
			if gotListAvailable != testCase.wantListAvailable {
				t.Fatalf("GetAvailableModels() available = %v, want %v; models = %#v", gotListAvailable, testCase.wantListAvailable, models)
			}
		})
	}
}
