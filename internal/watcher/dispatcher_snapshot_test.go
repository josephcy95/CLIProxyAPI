...
func TestRefreshAuthStatePreservesConcurrentProxyUpdate(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, errUpdate := manager.Update(context.Background(), patched)
			if errUpdate != nil {
				t.Fatal(errUpdate)
			}
			w.addOrUpdateClient(path)
			...
			if current.ProxyURL != testCase.newProxy {
				t.Errorf("runtime proxy_url = %q, want %q after applying queued snapshot", current.ProxyURL, testCase.newProxy)
			}
		})
	}
}
