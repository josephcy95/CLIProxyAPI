...
func TestEndToEndAuthFileReplacement_RestoresModelsInV1ModelsWithoutRestart(t *testing.T) {
	...
	// Fork keeps suspended models discoverable in /v1/models; routing still blocks.
	service.coreManager.RegisterExecutor(&syncTestExecutor{})
	if _, errExec := service.coreManager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-6-astra"}, cliproxyexecutor.Options{}); errExec == nil {
		t.Fatal("expected inference to fail while credential is unauthorized")
	}
...
func TestEndToEndAuthFilePatch_RestoresModelsInV1ModelsWithoutRestart(t *testing.T) {
	...
	// Fork keeps suspended models discoverable; assert routing is blocked instead.
	if _, errExec := service.coreManager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-6-astra"}, cliproxyexecutor.Options{}); errExec == nil {
		t.Fatal("expected inference to fail while credential is unauthorized")
	}
