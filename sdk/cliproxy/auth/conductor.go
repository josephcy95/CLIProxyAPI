	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				failed = true
				// resultErrorFromError preserves HTTP status + request-scoped flags;
				// RetryAfter keeps provider cooldowns (e.g. xAI free-usage 24h).
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			...
			if disabledByExhaustion {
				...
				// Upstream request-scoped + our not-found scoped errors both skip model cool.
...
func (m *Manager) persist(ctx context.Context, auth *Auth) error {
	...
	// Keep compact runtime cooldown/counters in the auth JSON metadata.
