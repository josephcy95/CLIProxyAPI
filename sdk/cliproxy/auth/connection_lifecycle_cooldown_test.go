...
func TestManager_MarkResult_HTTPStatusWithLifecycleTextStillCooldowns(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			m := NewManager(nil, nil, nil)
			// "openai" avoids the fork's Codex 401 auth-death policy, which disables the
			// credential instead of cooling the model; the HTTP-status-with-lifecycle-text
			// cooldown behavior is what this test exercises.
			auth := &Auth{ID: "auth-status-" + tc.name, Provider: "openai"}
			if _, errRegister := m.Register(context.Background(), auth); errRegister != nil {
				t.Fatalf("register auth: %v", errRegister)
			}
